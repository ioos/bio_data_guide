...
fish_data <- read_csv(here::here("data_to_dwc", "hakai_salmon_data", "raw_data", "fish_data.csv"))
```
...
The [Darwin Core](https://dwc.tdwg.org) is a highly structured data format that standardizes data table relations, vocabularies, and defines field names. The Darwin Core defines three table types: `event`, `occurrence`, and `measurementOrFact`. This intuitively captures the way most ecologists conduct their research. Typically, a survey (event) is conducted during which time some measurements, counts, or observations (collectively measurementOrFact[s]) are made regarding a specific habitat or species (occurrence). 

In the following script I demonstrate how I go about converting a subset of the data collected from the Hakai Institute Juvenile Salmon Program and discuss challenges, solutions, pros and cons, and when it might be worthwhile to convert to Darwin Core.

The conversion of a dataset to Darwin Core is much easier if your data is already tidy (normalized) in which you represent your data in separate tables that reflect the hierarchical and related nature of observations. If you're data is not already in a consistent and structured format, the conversion would likely be very arduos and not intuitive.
...
event <- tibble(eventID = seine_data$seine_id,
                ...
                samplingEffort = "NA", # No standardized effort can be applied to each event. Do not assume equal effort for each event 
...
fHere I convert length, and weight measurements that relate to an event and an occurrence and call those `measurementTypes` as `length` and `weight`. Species of salmon that are caught with each seine relate only to the event. I defined the `measurementType` as `sppCount`.
...
It is possible to re map a normalized data set into Darwin Core. My sense is it would be next to impossible, or at least very time consuming, to do this for data that are not currently in a normalized structure. You could, however, start to collect data in this format, but that would leave you unable to analyze the data using common tools in R. I'm not sure what the value is in converting data to this standard. If the argument is for making the data discoverable, I think there are more efficient ways to do that (data catalogues). If the argument is for standardizing data from different projects into one format for meta-analysis, I would argue that leads people down the wrong path because a scientist must inspect and understand very closely the protocols and methods used to produce the data. Heatmaps of speceies counts based on Darwin Data are not a scientific representation of species distributions, but rather a tool for looking a who has converted their data to Darwin Core and uploaded it to OBIS. By aggregating multiple data sets into a tool to view presence of species that were observed under different protocols still requires the analyst to understand deeply the research method, and whether species absence is truly absence. The canonical unit of data is still a data package with a DOI that provides methods, changelogs, data dictionaries, suppoerting documents, raw data, scripts to process raw data, and processed data. Darwin Core may have value if international working groups standardize `measurementTypes` and agree on the same protocols.