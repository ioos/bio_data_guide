...
fish_data <- read_csv(here::here("data_to_dwc", "hakai_salmon_data", "raw_data", "fish_data.csv"), guess_max = 20000) %>% semi_join(survey_seines)
...
fish_data <- fish_data %>%
  select(-fork_length_field, -weight_field) %>% 
  drop_na(weight) %>% 
...
Here I convert length, and weigh measurements that relate to an event and an occurrence and call those `measurementTypes` as `length` and `weight`. Species of salmon that are caught with each seine relate only to the event. I defined the `measurementType` as `sppCount`.

```{r measurementOrFact}

# First I tidy the fish abundance data into a long format so that it is a measurement that relates to an event
fish_abund <- survey_seines %>% 
  gather(`so_total`, `pi_total`, `cu_total`, `co_total`, `he_total`, `ck_total`, key = scientificName, value = individualCount) %>% 
  drop_na(individualCount) %>% 
  select(eventID = seine_id, scientificName, individualCount) %>% 
  mutate(occurrenceID = NA,
         ...
         measurementRemarks = "visual estimate"
         ) %>% 
  select(eventID, occurrenceID, scientificName, individualCount, measurementType, measurementValue, measurementValue, measurementTypeID, measurementAccuracy, measurementUnit, measurementUnitID, measurementRemarks)

fish_abund$scientificName <- factor(fish_abund$scientificName) %>% 
  recode(so_total = "Oncorhynchus nerka",
