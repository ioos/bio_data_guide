...
#' swith (ie. 10-15m will become 15m). To account for no preceeding distance,
...
seagrass <- 
  ...
  select( - c(X.x, X.y, hakai_id.x, hakai_id.y,  # remove unnecessary variables
              ...
              depth.x, depth.y,
              sampling_bout.x, sampling_bout.y)) %T>%
  glimpse()
...
seagrassOccurrence <-
  seagrass %>%
  select(eventID, site_id, transect_dist, occurrenceID) %>%
  mutate(scientificName   = "Zostera subg. Zostera marina",
...
seagrassOccurrence <- 
  full_join(seagrassOccurrence, zmWorms, 
            by = c("scientificName" = "ScientificName")) %T>%
  glimpse
...
seagrassMof <-
  ...
  select(date,
         ...
         density_msq, canopy_height_cm, flowering_shoots, depth_m) %>%
  ...
  mutate(
    ...
    measurementMethod = case_when(
      ...
      measurementType == "FloweringBedAbund" ~ "25cmx25cm quadrat count")) %T>%
  glimpse()
